...
Be sure you have correctly installed the `cf` CLI (or Command Line Interface): https://github.com/cloudfoundry/cli. This is the tool you will use to interact with Cloud Foundry. You can test that it works by running `cf version`:
...
```
$ cf login -u <USERNAME> -p <PASSWORD> -a <CLOUDFOUNDRY_API_ENDPOINT>
```

Be sure to use your values in the command above.  The `-a` flag specifies the API endpoint for the Cloud Foundry instance you are using. For example, if you are using Pivotal Web Services the `<CLOUDFOUNDRY_API_ENDPOINT>` is `api.run.pivotal.io`.
...
Now that you are logged in, you can deploy an application. In Cloud Foundry terms, this is a `cf push`. You will be pushing a java application packaged as a jar file. You can download the jar file here: https://drive.google.com/open?id=1s3O1RvCgLdFWHaDT5Dglb_jZP8-P-fMB
...
The Cloud Foundry marketplace is a collection of services that can be provisioned on demand.  Your marketplace may differ depending on the Cloud Foundry distribution you are using.  Find the MySQL service in your marketplace. If you are using Pivotal Web Services, you will be using a MySQL service from `cleardb`.
...
> Note: You can list all available services in the marketplace by running `cf marketplace`. The service name and plan for MySQL in your Cloud Foundry instance will be different if you aren't using Pivotal Web Services.
